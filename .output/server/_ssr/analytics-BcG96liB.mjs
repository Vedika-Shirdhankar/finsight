//#region node_modules/.nitro/vite/services/ssr/assets/analytics-BcG96liB.js
/**
...
*/
function detectAnomalies(transactions, categoryNameById, opts = {}) {
	...
	for (const [key, group] of groups) {
		...
		for (const t of group) {
			...
			if (z >= zThreshold) anomalies.push({
				transaction: t,
				zScore: z,
				categoryLabel: key === "__uncategorized__" ? "Uncategorized" : categoryNameById.get(key) ?? "a category"
			});
		}
	}
...
*/
function forecastNextMonthSpend(transactions, windowMonths = 3) {
	...
	const history = Array.from(byMonth.keys()).sort().map((month) => ({
		month,
	...
	return {
		history,
		projectedNextMonth,
		trendPct: lastMonth > 0 ? (projectedNextMonth - lastMonth) / lastMonth * 100 : 0,
		hasEnoughData: true
	};
}
//#endregion
export { forecastNextMonthSpend as n, detectAnomalies as t };
