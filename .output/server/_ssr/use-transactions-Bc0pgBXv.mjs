...
//#region node_modules/.nitro/vite/services/ssr/assets/use-transactions-Bc0pgBXv.js
function useTransactions(userId, filters = {}) {
	return useQuery({
...
function useCreateTransaction(userId) {
	...
	return useMutation({
		mutationFn: async ({ splits, ...input }) => {
			...
			if (error) throw error;
			if (splits?.length) {
				const { error: splitError } = await supabase.from("transaction_splits").insert(splits.map((split) => ({
					...split,
...
function useUpdateTransaction(userId) {
	const queryClient = useQueryClient();
	return useMutation({
...
function useDeleteTransaction(userId) {
	const queryClient = useQueryClient();
	return useMutation({
		mutationFn: async (id) => {
...
//#endregion
export { useUpdateTransaction as i, useDeleteTransaction as n, useTransactions as r, useCreateTransaction as t };
